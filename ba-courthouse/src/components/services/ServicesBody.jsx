...
const CoreServices = () => {
  ...
  const services = [
   ...
   const containerVariants = {
    ...
    visible: {
      opacity: 1,
      transition: {
        staggerChildren: 0.1, // Reduced stagger for quicker appearance
        delayChildren: 0.1,  // Reduced initial delay
        ease: "easeInOut"     // Smoother easing for container
      }
    }
  };
  ...
  const itemVariants = {
    hidden: { 
      opacity: 0, 
      y: 20,      // Reduced initial y-offset for subtler entrance
      scale: 0.98 // Slight scale down for depth effect
    },
    visible: {
      opacity: 1,
      y: 0,
  ...
  const hoverVariants = {
    hover: {
      y: -8,               // Slightly more pronounced hover effect
      transition: {
        type: "spring",
        damping: 10,
        stiffness: 300,    // Snappier hover response
        mass: 0.5         // Controls weight of the motion
      }
    }
  };
